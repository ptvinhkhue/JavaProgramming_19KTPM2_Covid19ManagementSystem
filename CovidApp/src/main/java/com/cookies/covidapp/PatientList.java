...
public class PatientList extends JPanel {
    ...
    public PatientList(int n, ArrayList<String> username, ArrayList<String> name, ArrayList<String> year) {
        super();
        ...
        for (int i = 0; i < n; i++) {
            PatientListItem pli_cur = new PatientListItem(username.get(i), name.get(i), year.get(i));
            pli.add(pli_cur);
            table.add(pli_cur);
        }
